...
async function logoutUser(page) {
  ...
  try {
    await page.waitForURL(url => url.pathname.includes('/Account/Login') || url.pathname === '/' || url.pathname === '', { timeout: 10000 });
  } catch (err) {
...
test('Real Estate Complete End-to-End Workflow (UC 06 - UC 20) - 4K Video Presentation', async ({ page }) => {
  test.setTimeout(420000); // 7 minutes timeout for human-paced flow
  ...
  try {
    ...
    await delay(1500);
    await page.getByRole('button', { name: 'Sign In' }).click();
    await page.waitForURL(/.*\/RealEstate\/Inbox/, { timeout: 15000 });
    await delay(2000);
    ...
    // ========================================================
    console.log('STEP 18: Admin logs in to conduct unit inspection (UC 14)...');
    await page.locator('#UserName').fill('BOSystemAdminstrator');
    await delay(1000);
    await page.locator('#Password').fill('Arsenal5@');
    await delay(1500);
    await page.getByRole('button', { name: 'Sign In' }).click();
    await page.waitForURL(/.*\/AreaManager\/AdminInbox/, { timeout: 25000 });
    await delay(2000);
    ...
    console.log('Logging off Administrator session...');
    await logoutUser(page);
    await delay(3000);
    ...
    await delay(1500);
    await page.getByRole('button', { name: 'Sign In' }).click();
    await page.waitForURL(/.*\/RealEstate\/Inbox/, { timeout: 25000 });
    await delay(2000);
    ...
    // ========================================================
    console.log('STEP 26: Administrator logs in to review PTO request (UC 19)...');
    await page.locator('#UserName').fill('BOSystemAdminstrator');
    await delay(1000);
    await page.locator('#Password').fill('Arsenal5@');
    await delay(1500);
