...
{
    [Authorize]
    public class PLMReportController : Controller
    {
        ...
        public PLMReportController()
       : this(new UserManager<SystemIdentityUser>(new UserStore<SystemIdentityUser>(new eServicesDbContext())))
        {

        }
        public PLMReportController(UserManager<SystemIdentityUser> userManager)
        {
            UserManager = userManager;
        }
        public PLMReportController(eServicesDbContext db)
        {
            UserManager =
        ...
        public ActionResult GenerateDateRangeReport(string startDate, string endDate, string reportId, string docTypeId, string RegionType, string StatusType, string Rejected)
        {
            ...
            WebClient client = new WebClient();
            client.Credentials = nwc;
            DateTime StartDate = Convert.ToDateTime(startDate != null ? startDate : DateTime.Now.ToString());
            DateTime EndDate = Convert.ToDateTime(endDate != null ? endDate : DateTime.Now.ToString());
